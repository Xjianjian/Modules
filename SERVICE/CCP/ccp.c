...
void ccpSetMTAAddr(CCP_BYTE mta, CCP_MTABYTEPTR addr)
{
	if(mta < CCP_MAX_MTA)
	{
		ccpSetMTA(mta,addr);
...
CCP_DWORD ccp_getMTAAddr(CCP_BYTE mta)
{
	CCP_DWORD addr;
	if(mta < CCP_MAX_MTA)
	{
		addr = (CCP_DWORD)(ccp.MTA[mta]);
	}
	return addr;
}
...
void ccpCommand( CCP_BYTEPTR com ) {
  ...
  /* Handle other commands only if connected */
    ...
    switch (cmd) {
      ...
      case CC_START_STOP_ALL: /* Cyclic acquisition start/stop */
        {
          #define ssCmd                 com[2]  /* Start or Stop */
