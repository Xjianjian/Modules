 ...
 * REQ IDs:
 * History:
 * 2016-08-29, Devon Draft version
 */
...
#define m_dataLinkDlcCare 	   FALSE
...
#include "dataLinkCfg.h"
#include "dataLinkPri.h"
#include "dataLink.h"
...
uint8_t CANILModeGrp[m_canMod_cnt] =
{
	CAN_IL_REC_SEND_STOP,CAN_IL_REC_SEND_STOP,CAN_IL_REC_SEND_STOP
};
...
void hcan_init(void)
{
	...
	//{
	//	CANILModeGrp[bus_i] = CAN_IL_RS_STOP;
	//}
...
void hcan_setILMode(uint8_t module,uint8_t mode,uint8_t set)
{
	if(set)
	{
		CANILModeGrp[module] |= mode;
	}
	else
	{
		CANILModeGrp[module] &= ~mode;
...
void hcan_canScanRecMsgDispose(uint8_t module,uint8_t mbIndx,uint32_t id,uint8_t * buf,uint8_t dlc)
{
		...
		(TRUE == hcanMboxAssignGrp[module].mboxAssignedOk) && (mbIndx < canMsgCfgInfoGrp[module].msgNum))
	{
		...
		if((FALSE == cfg.isrEn) && (m_can_dirRx == cfg.dir) && \
			(id == (cfg.id  | extern_id)))
		{
			canMsgScheCfgInfoGrp[module][mbIndx].timeRemain = cfg.period + 1;
			/*modified based on NDS 06 rules, interaction-presentation layer*/
#if m_dataLinkDlcCare
	...
	if((module < m_canMod_cnt) && (TRUE == hcanMboxAssignGrp[module].mboxAssignedOk))
	{
		if(m_hcan_mboxFix == hcanMboxAssignGrp[module].MboxAssignWay)
		{
			if(mbIndx < canMsgCfgInfoGrp[module].msgNum)
			{
				...
				{
					if((m_can_norMsg != cfg.msgType) && (dlc != 8))
					{
						return;
					}
...
void hcan_canIsrTxConfirmCb(uint8_t module,uint8_t mbIndx,uint32_t id)
{
	...
	if(module < m_canMod_cnt)
	{
		hcan_canBusOffRemoveCb(module);
		pCfg = canMsgCfgInfoGrp[module].pCfg;
		if(NULL != pCfg)
		{
			if(m_hcan_mboxFix == hcanMboxAssignGrp[module].MboxAssignWay)
			{
				if(module == ccpTxCanBus && DTLINK_CCP_TX_MB == mbIndx)
				{
					canMsgScheCfgInfoGrp[module][ccpTxMsgIndx].txToCnt = 0;
					if(NULL != pCfg[ccpTxMsgIndx].txSuccessCb)
					{
						pCfg[ccpTxMsgIndx].txSuccessCb();
					}
				}
				else if(mbIndx < canMsgCfgInfoGrp[module].msgNum)
				{
					canMsgScheCfgInfoGrp[module][mbIndx].txToCnt = 0;
					can_clrBusOffFlag(module);
					if(NULL != pCfg[mbIndx].txSuccessCb)
					{
						pCfg[mbIndx].txSuccessCb();
					}
				}	
			}
			else
			{
				...
				else if((m_dtLink_dynTxMb == mbIndx) && (NULL != canTxDynMsgIndxBuf[module]))
				{
#if m_dtLink_DyntxCycleFifo
...
{
	sm_busoffCb(module);
	hcan_canBusInitReq(module);
}

void hcan_canBusOffRemoveCb(uint8_t module)
{
	sm_busoffRecoverCb(module);
}
				   ...
				   (m_can_dirRx == cfg.dir))
		{
            ...
            if(NULL != cfg.received)
            {
                *(cfg.received) = FALSE;
	   			...
	   			(m_can_dirRx == cfg.dir))
		{
			autosar_dirNm_rec_indication(module,cfg.id,cfg.dlc, cfg.buf);
			if(NULL != cfg.received)
			{
				*(cfg.received) = FALSE;
			}
...
bool hcan_ccpSendReq(uint8_t * msg)
{
	...
	const volatile canMboxCfg_t *pCfg = NULL;
	bool txOk = FALSE;
	if((ccpTxCanBus < m_canMod_cnt)  && (CAN_IL_CCP_SEND & CANILModeGrp[ccpTxCanBus]) && (ccpTxMsgIndx < canMsgCfgInfoGrp[ccpTxCanBus].msgNum) &&
	  (m_can_dirTx == canMsgCfgInfoGrp[ccpTxCanBus].pCfg[ccpTxMsgIndx].dir) && (m_can_ccpMsg == canMsgCfgInfoGrp[ccpTxCanBus].pCfg[ccpTxMsgIndx].msgType))
	{
            ...
            if(NULL != pCfg[ccpTxMsgIndx].txRdy)
            {
                ...
                txOk = can_txMsgFix(ccpTxCanBus,DTLINK_CCP_TX_MB,pCfg[ccpTxMsgIndx].id, pCfg[ccpTxMsgIndx].buf, pCfg[ccpTxMsgIndx].dlc);
                *(pCfg[ccpTxMsgIndx].txRdy) = FALSE;
            }
	}
	return txOk;
}
...
bool hcan_ntTransmit(uint8_t * msg)
{
	...
	bool txOk = FALSE;
	const volatile canMboxCfg_t *pCfg = NULL;
	if((udsTxCanBus < m_canMod_cnt) && (CAN_IL_UDS_SEND & CANILModeGrp[udsTxCanBus]) && (udsTxMsgIndx < canMsgCfgInfoGrp[udsTxCanBus].msgNum) &&
			...
			if(m_hcan_mboxFix == hcanMboxAssignGrp[module].MboxAssignWay)
			{
				...
				for(msgIndx = 0;msgIndx < canMsgCfgInfoGrp[module].msgNum;msgIndx ++)
				{
					...
					if(m_can_dirRx == canMsgCfgInfoGrp[module].pCfg[msgIndx].dir)
					{
						...
						canMsgScheCfgInfoGrp[module][msgIndx].timeRemain = canMsgCfgInfoGrp[module].pCfg[msgIndx].period + 1;
					}
					else
					{
                ...
                if(ccpTxCanBus == module)
                {
                    can_txMbSet(module,DTLINK_CCP_TX_MB,0,dlc,m_can_txInactive);
                    can_mbIntCmd(module,DTLINK_CCP_TX_MB,em_funSt_on);
                }
            }
			else
			{
		...
		for(sort_i = 0;sort_i < dtLinkMsgTypeCollectGrp[module].recMsgCnt; sort_i ++)
		{
			   ...
			   (m_hcan_mboxDyna == hcanMboxAssignGrp[module].MboxAssignWay))
			{
				...
				else
				{
					...
					if((m_can_periodMsg == canMsgCfgInfoGrp[module].pCfg[msgIndx].msgMode || m_can_peMsg == canMsgCfgInfoGrp[module].pCfg[msgIndx].msgMode)
							&& canMsgCfgInfoGrp[module].pCfg[msgIndx].period > 0)
					{
			...
			else
			{
				...
				if(TRUE == canMsgScheCfgInfoGrp[module][msgIndx].msgRec)
				{
					...
					canMsgScheCfgInfoGrp[module][msgIndx].timeRemain = canMsgCfgInfoGrp[module].pCfg[msgIndx].period + 2;
					hcan_canRxIndiCb(module,msgIndx);
				}
				else
				{
					...
					//if(canMsgCfgInfoGrp[module].pCfg[msgIndx].period > 0)
					{
						...
						else
						{
							if(NULL != canMsgCfgInfoGrp[module].pCfg[msgIndx].rxTimeout)
							{
								if(FALSE == *(canMsgCfgInfoGrp[module].pCfg[msgIndx].rxTimeout))
								{
									hcan_canRxToCb(module,msgIndx);
									*(canMsgCfgInfoGrp[module].pCfg[msgIndx].rxTimeout) = TRUE;
								}
	...
	if((module < m_canMod_cnt) && (NULL != canMsgCfgInfoGrp[module].pCfg))
	{
		...
		if(m_hcan_mboxFix == hcanMboxAssignGrp[module].MboxAssignWay)
		{
			for(sort_i = dtLinkMsgTypeCollectGrp[module].recMsgCnt;sort_i < dtLinkMsgTypeCollectGrp[module].memCnt; sort_i ++)
			{
				...
				if((m_can_nmMsg == pCfg[msgIndx].msgType) || (CAN_IL_APP_SEND & CANILModeGrp[module]))
				{
				...
				if(FALSE == canMsgCfgInfoGrp[module].pCfg[msgIndx].isrEn)
				{
					...
					if(canMsgScheCfgInfoGrp[module][msgIndx].txToCnt > 0)
					{
						if(m_can_istxDtFinshCode(mbCode))
						{
							canMsgScheCfgInfoGrp[module][msgIndx].txToCnt = 0;
							can_clrBusOffFlag(module);
							if(NULL != canMsgCfgInfoGrp[module].pCfg[msgIndx].txSuccessCb)
							{
